    ...
    function compare(a: Comparable, b: Comparable): boolean {
        ...
        if (a instanceof Map) {
            ...
            for (const [key, val] of a.entries()) {
                if (!b.has(key) || !compare(val as Comparable, b.get(key) as Comparable))
                    return false;
            }
        ...
        if (a.constructor === Object && b.constructor === Object) {
            ...
            for (const key of keysA) {
                if (!(key in objB) || !compare(objA[key] as Comparable, objB[key] as Comparable)) {
                    return false;
                }
            }
...
export function memoize<T>(fn: (...args: unknown[]) => T): (...args: unknown[]) => T {
    ...
    return (...args: unknown[]): T => {
        ...
        try {
            ...
            key = String(args.length) + args.map((arg, i) => `${i}:${typeof arg}`).join(",");
        }
