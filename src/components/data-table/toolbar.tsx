...
export function DataTableToolbar<TData>({
    ...
    const [localSearch, setLocalSearch] = useState(decodedSearchParam || tableSearch);
    const isLocallyUpdatingSearch = useRef(false);

    const computeDatesFromUrl = useCallback((): {
        from: Date | undefined;
        to: Date | undefined;
    } => {
    ...
    const datesModified = !!dates.from || !!dates.to;
            ...
            <div className="flex flex-1 flex-wrap items-center gap-2">
                {config.enableSearch && (
                    <Input
                        ...
                        className={`${getInputSizeClass(config.size)} h-8 w-[150px] lg:w-[250px]`}
                    />
                )}
