...
export function ExportResultsModal({
  ...
  const [fileName, setFileName] = useState(
    `${reviewName.replace(/\s+/g, "_")}_Results_Export`,
  );
  ...
  const toggleAllInCategory = (category: string) => {
    ...
    const allSelected = categoryColumns.every((id) =>
      selectedColumns.includes(id),
    );

    if (allSelected) {
      setSelectedColumns((prev) =>
        prev.filter((id) => !categoryColumns.includes(id)),
      );
    } else {
  ...
  const toggleAllBreakdownCriteria = () => {
    ...
    const allSelected = allCriteriaIds.every((id) =>
      breakdownCriteria.includes(id),
    );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[55vw] min-w-[55vw] max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="flex-shrink-0 px-6 pt-6">
          ...
          <DialogDescription>
            Configure your export settings and select which data to include in
            the Excel file.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6 py-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <Badge
                    variant="secondary"
                    className="h-10 items-center px-3 whitespace-nowrap flex-shrink-0"
                  >
                    .xlsx
                  </Badge>
                </div>
              ...
              <div className="space-y-3">
                <Label className="flex items-center gap-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>
                        Filter which students/teams to include based on batch or
                        course
                      </p>
                    </TooltipContent>
                  </Tooltip>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                      {[
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setGroupBy(option.value as ExportConfig["groupBy"])
                          }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                      {[
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setSortBy(option.value as ExportConfig["sortBy"])
                          }
                          className={`relative flex items-start gap-3 p-3 rounded-lg border-2 transition-all text-left min-w-0 ${
                            sortBy === option.value
                              ? "border-primary bg-primary/5"
                ...
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>
                          Choose which data fields to include as columns in the
                          Excel file
                        </p>
                      </TooltipContent>
                    </Tooltip>
                      ...
                      <div className="flex-1">
                        ...
                        >
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>
                                Creates a detailed breakdown view showing how
                                marks are distributed across criteria
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </Label>
                        <p className="text-sm text-muted-foreground mt-1">
                          Display detailed score breakdown for each criterion in
                          the export
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3 pl-7 border-l-2 border-primary/20">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant="secondary">
                              {breakdownCriteria.length} / {criteria.length}{" "}
                              selected
                            </Badge>
                            <Button
                              variant="ghost"
                          ...
                          {criteria.map((criterion) => (
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={() =>
                                  toggleBreakdownCriteria(criterion.id)
                                }
                                className="flex-shrink-0"
                              />
                              <Label
                    ...
                    <div>
                      <Label className="flex items-center gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>
                              Set fallback values to use when data is missing or
                              unavailable
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      ...
                      {selectedColumns.map((columnId) => {
                        ...
                        return (
                          <div
                            ...
                            className="flex items-center gap-3 p-2 rounded-lg border bg-muted/10"
                          >
                            <Label className="flex-shrink-0 w-48 text-xs">
                              {columnNames[columnId]}
                            </Label>
                            <Input
                              value={columnDefaultValues[columnId] || ""}
                              onChange={(e) =>
                    ...
                    <div>
                      <Label className="flex items-center gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>
                              Drag and drop to reorder columns in the Excel
                              export
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Drag to reorder how columns will appear in the export
                      </p>
                    </div>
                    ...
                    >
                      <SortableContext
                        items={columnOrder.filter((id) =>
                          id === "BREAKDOWN"
                            ? showMarkBreakdown && breakdownCriteria.length > 0
                            : selectedColumns.includes(id),
                        )}
                        ...
                        <div className="space-y-2 p-1">
                          {orderedColumns.map((item, index) => (
                            ...
                            >
                              <Badge
                                variant="outline"
                                className="h-6 w-6 flex-shrink-0 flex items-center justify-center text-xs"
                              >
                                {index + 1}
                              </Badge>
                              <SortableColumnItem
                                ...
                                type={item.type}
                                criteria={criteria}
                                breakdownCriteria={breakdownCriteria}
                              />
                            </div>
                          ))}
                        </div>
        ...
        <DialogFooter className="flex-shrink-0 flex items-center gap-2 px-6 py-4 border-t">
