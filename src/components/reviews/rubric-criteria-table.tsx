...
export function RubricCriteriaTable({
    ...
    const [scoreInputs, setScoreInputs] = useState<string[]>(() =>
        criteria.map((c) => String(c.maxScore))
    );
    ...
    if (criteria.length === 0 && isReadOnly) {
        return (
            <div className="p-8 text-center border-dashed border-2 rounded-lg">
                ...
                <p className="text-muted-foreground mb-4">This rubric has no criteria defined.</p>
            </div>
        );
    }

    const totalScore = criteria.reduce((total, criterion) => total + (criterion.maxScore || 0), 0);
    ...
    return (
        <div className="w-full overflow-auto rounded-md border">
            ...
            <div className="flex justify-between items-center p-2 border-t bg-muted/50">
                ...
                <div className={`font-semibold text-right pr-4 ${isReadOnly ? "w-full" : "w-1/2"}`}>
