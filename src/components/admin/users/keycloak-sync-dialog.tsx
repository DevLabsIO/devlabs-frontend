...
export function KeycloakSyncDialog({
  isOpen,
  onClose,
  onSyncComplete,
}: KeycloakSyncDialogProps) {
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>([]);
  const { error: toastError, success: toastSuccess } = useToast();

  const { data: syncStats, isLoading, refetch } = useSyncStats();
  const syncMutation = useSyncFromKeycloak();

  const handleSelectAll = (checked: boolean) => {
    if (checked && syncStats?.unsyncedUsers) {
      setSelectedUserIds(syncStats.unsyncedUsers.map((u) => u.id));
    } else {
      setSelectedUserIds([]);
    }
  };
  ...
  const handleSync = async () => {
    if (selectedUserIds.length === 0) {
      toastError("Please select at least one user to sync");
      return;
    }
    ...
    syncMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          toastSuccess(data.message || "Users synced successfully");
          setSelectedUserIds([]);
          refetch();
          onSyncComplete?.();
        },
        onError: (err: Error) => {
          toastError(err.message || "Failed to sync users");
        },
      },
    );
  };
  ...
  const handleSyncAll = async () => {
    syncMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          toastSuccess(data.message || "All users synced successfully");
          setSelectedUserIds([]);
          refetch();
          onSyncComplete?.();
        },
  ...
  const getRoleFromGroups = (groups: string[]) => {
    if (groups.includes(KEYCLOAK_GROUPS.ADMIN)) return "Admin";
    if (groups.includes(KEYCLOAK_GROUPS.MANAGER)) return "Manager";
    if (groups.includes(KEYCLOAK_GROUPS.FACULTY)) return "Faculty";
    if (groups.includes(KEYCLOAK_GROUPS.STUDENT)) return "Student";
    return "Student";
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-7xl max-h-[85vh] flex flex-col p-0 gap-0">
        <div className="relative bg-gradient-to-r from-blue-600 via-purple-600 to-cyan-600 dark:from-blue-700 dark:via-purple-700 dark:to-cyan-700 px-6 py-8 rounded-t-lg">
          ...
          <div className="relative z-10">
            <DialogHeader>
              ...
              <DialogDescription className="text-blue-50 mt-2">
                Import and synchronize users from your Keycloak identity
                provider
              </DialogDescription>
            </DialogHeader>
          </div>
