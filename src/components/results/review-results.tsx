...
export function ReviewResults({ reviewId, projectId }: ReviewResultsProps) {
    ...
    const {
        ...
        queryFn: () => resultQueries.getResults(reviewId, projectId, currentUser?.id || ""),
    ...
    if (error) {
        ...
        const errorMessage = axiosError?.response?.data?.error || "Failed to load results";

        if (axiosError?.response?.status === 403 || errorMessage.includes("published")) {
            return (
        ...
        return (
            <div className="space-y-4 p-6 border rounded-lg bg-background">
                ...
                <Alert variant="destructive">
                    <AlertDescription>Unable to load results. {errorMessage}</AlertDescription>
                </Alert>
            </div>
        );
    }
    ...
    const getPerformanceIcon = (percentage: number) => {
        ...
        if (percentage >= 70) return <BarChart3 className="h-4 w-4 text-yellow-600" />;
