...
export default function PerformanceChart({ data }: PerformanceChartProps) {
    const courseColors = ["#3b82f6", "#ef4444", "#10b981", "#f59e0b", "#8b5cf6", "#ec4899"];

    const allDates = Array.from(
        new Set(data.flatMap((course) => course.reviewHistory.map((review) => review.reviewDate)))
    ...
    const chartData = allDates.map((date) => {
        ...
        data.forEach((course) => {
            ...
            if (review) {
                dataPoint[course.name] = (review.averageScore / review.maxPossibleScore) * 100;
            }
        });
