...
import { CourseEvaluationData, IndividualScoreSubmission } from "@/types/features";
...
export function CourseEvaluationForm({
    ...
    const initializeCommonCriteriaData = (): CommonCriteriaData => {
        ...
        evaluationData.criteria.forEach((criterion) => {
            if (isCriterionCommon(criterion.id)) {
                ...
                const existingScore = sourceScores?.find((cs) => cs.criterionId === criterion.id);
    ...
    const [commonCriteriaData, setCommonCriteriaData] = useState<CommonCriteriaData>(
        initializeCommonCriteriaData
    );
    ...
    const handleDraftLoaded = useCallback(
            ...
            draftScores.forEach((participantScore) => {
                if (newFormData[participantScore.participantId]) {
                    participantScore.criterionScores.forEach((criterionScore) => {
                        if (
                            newFormData[participantScore.participantId][criterionScore.criterionId]
                        ) {
        ...
        [evaluationData, isCriterionCommon, setFormData, setCommonCriteriaData]
    );

    const { isDraftLoading, isSaving, lastSaveTime, saveDraft } = useEvaluationDraft({
        reviewId,
        projectId,
    ...
    useEffect(() => {
        if (!isDraftLoading && !isDraftLoadedRef.current) {
            isDraftLoadedRef.current = true;
        }
    }, [isDraftLoading]);

    const convertFormDataToDraftScores = useCallback((): ParticipantScoreData[] => {
    ...
    const submitMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({
                queryKey: ["courseEvaluationData", reviewId, projectId, evaluationData.courseId],
            });
            queryClient.invalidateQueries({
                queryKey: ["evaluationSummary", reviewId, projectId],
            });
            queryClient.invalidateQueries({
                queryKey: ["evaluationDraft", reviewId, projectId, evaluationData.courseId],
            });
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div className="space-y-1">
                            <div className="flex items-center gap-3">
                                <h2 className="text-2xl font-bold">{evaluationData.courseName}</h2>
                                {!evaluationData.isPublished && (
            ...
            {evaluationData.isPublished && (
                <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20 dark:border-blue-900">
                    ...
                    <AlertDescription className="text-blue-800 dark:text-blue-200">
                        This evaluation has been published. You can still edit and save changes as
                        an authorized user. Changes will be reflected in the results immediately.
                    </AlertDescription>
                </Alert>
            )}

            <div className="grid gap-4 md:grid-cols-3">
                <Card className="border-2 bg-linear-to-br from-blue-50 to-background dark:from-blue-950/20 dark:to-background">
                ...
                <Card className="border-2 bg-linear-to-br from-violet-50 to-background dark:from-violet-950/20 dark:to-background">
                    <CardHeader className="pb-3">
                        <div className="flex items-center gap-3">
                ...
                <Card className="border-2 bg-linear-to-br from-emerald-50 to-background dark:from-emerald-950/20 dark:to-background">
                    <CardHeader className="pb-3">
                        <div className="flex items-center gap-3">
            ...
            <Card className="border-2 bg-linear-to-r from-background to-muted/20 sticky bottom-4 shadow-lg">
