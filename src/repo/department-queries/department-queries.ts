...
export const useUpdateDepartment = () => {
    ...
    return useMutation({
        mutationFn: async (data: UpdateDepartmentRequest): Promise<DepartmentResponse> => {
            ...
            const response = await axiosInstance.put(`/api/department/${id}`, updateData);
            return response.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["departments"] });
...
export const useGetDepartmentBatches = () => {
    return useMutation({
        mutationFn: async (departmentId: string) => {
            const response = await axiosInstance.get(`/api/department/${departmentId}/batches`);
            return response.data;
        },
    });
};
...
export const getBatchesByDepartment = async (departmentId: string): Promise<Batch[]> => {
    const response = await axiosInstance.get(`/api/department/${departmentId}/batches`);
    return response.data;
};
